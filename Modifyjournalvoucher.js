...
const [showModifyDeleteModal, setShowModifyDeleteModal] = useState(false); // Modal for modify/delete option
  ...
  const insertData = async () => {
    ...
    if (!ojvno || rows.length === 0) {
      Alert.alert("Validation Error", "Please provide JV No and rows.");
      return;
    }
    ...
    fetch("https://5461283ce09c.ngrok-free.app/myapp/insertmodifyjv.php", {
      method: "POST",
      headers: {
  ...
  const addFetchedRows = (fetchedData) => {
    ...
    calculateTotals(updatedRows); // Update totals
  };
  ...
  const fetchData = () => {
    if (!ojvno) {
      Alert.alert("Validation Error", "Please enter an JV No.");
      return;
    }
  
    fetch("https://5461283ce09c.ngrok-free.app/myapp/fetchjvdata.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
>
  ...
  <Text style={styles.heading}>Modify Journal Voucher</Text>
</View>
  ...
  renderItem={({ item }) => (
    <TouchableOpacity onPress={() => handleRowClick(item)} style={styles.tableRow}>
      <Text style={styles.tableRowText}>{item.accountNo}</Text>  // Debugging display
      <Text style={styles.tableRowText}>{item.accountName}</Text>  // Debugging display
      <Text style={styles.tableRowText}>{item.description}</Text>  // Debugging display
...
export default Modifyjournalvoucher;
