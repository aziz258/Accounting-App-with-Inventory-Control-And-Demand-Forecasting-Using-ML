...
const Modifycashreceivevoucher = () => {
...
const [showModifyDeleteModal, setShowModifyDeleteModal] = useState(false); // Modal for modify/delete option
  ...
  const insertData = async () => {
    ...
    if (!ojvno || rows.length === 0) {
      Alert.alert("Validation Error", "Please provide RV No and at least one row.");
      return;
    }
    ...
    const payload = {
      ...
      rows: rows.map((row) => ({
        ...
        debit: parseFloat(row.receipts) || 0, // 🔄 Changed `amount` to `debit`
      })),
    };
  
    fetch("https://5461283ce09c.ngrok-free.app/myapp/insertmodifyrvno.php", {
      method: "POST",
      headers: {
  ...
  const addRow = () => {
    ...
    const newRow = {
      ...
      accountName,
      description,
      receipts: parseFloat(amount), // Store amount in receipts column
    };
  ...
  const handleRowClick = (row) => {
    ...
    setAmount(row.receipts.toString()); // Set amount as receipts (convert to string)
  ...
  const fetchData = () => {
      if (!ojvno) {
        Alert.alert("Validation Error", "Please enter an OJV No.");
        return;
      }
    
      fetch("https://5461283ce09c.ngrok-free.app/myapp/fetchrvnodata.php", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ ojvno }), // Send OJV No to PHP
      })
        .then((response) => response.json())
        .then((data) => {
          console.log("Fetched Data:", data.records); // Debugging
          if (data.success) {
            addFetchedRows(data.records); // Use the function to display fetched data
            Alert.alert("Success", "Data fetched successfully.");
          } else {
            Alert.alert("Error", data.error || "No data found.");
          }
        })
        .catch((error) => {
    ...
    const addFetchedRows = (fetchedData) => {
        const updatedRows = fetchedData.map((rowData, index) => ({
          id: `${rowData.acct_no}-${index}`, // Unique ID based on acct_no and index
          accountNo: rowData.acct_no || "",  
          accountName: rowData.acct_name || "", // Keep empty if not provided
          description: rowData.des || "",  
          receipts: parseFloat(rowData.credit) || 0,
        
        }));
      
        setRows(updatedRows); // Replace table data with fetched data
...
>
  ...
  <Text style={styles.heading}>Modify Cash Receipt Voucher</Text>
</View>
    ...
    <View style={styles.row}>
      <View style={styles.inputContainer}>
        <Text style={styles.label}>RV No:</Text>
        <TextInput 
              style={styles.input} 
              value={ojvno} 
              onChangeText={setOjvno} 
            />
      </View>
    ...
    <View style={styles.row}>
     ...
     {/* Add Row Button */}
      ...
      {/* Fetch Data Button (Right-Aligned) */}
      <TouchableOpacity onPress={fetchData} style={styles.fetchDataButton}>
        <Text style={styles.fetchDataButtonText}>Fetch Data</Text>
      </TouchableOpacity>
    </View>
    ...
    <View style={styles.tableHeader}>
      ...
      <Text style={styles.tableHeaderText}>Receipts</Text>
    </View>
    <FlatList
      data={rows}
      keyExtractor={(item, index) => index.toString()}
      renderItem={({ item }) => (
        <TouchableOpacity onPress={() => handleRowClick(item)} style={styles.tableRow}>
          ...
          <Text style={styles.tableRowText}>{item.receipts}</Text>
        </TouchableOpacity>
        )}
      />
    </View>
...
const styles = StyleSheet.create({
  ...
  buttonContainer: {
    flexDirection: "row",
    justifyContent: "space-between", // Align buttons at opposite ends
    alignItems: "center",
    marginVertical: 10,
  ...
  addRowButton: {
    ...
    justifyContent: "center",
    alignItems: "center",
    flex: 1, // Allows button to take equal width
...
export default Modifycashreceivevoucher;
