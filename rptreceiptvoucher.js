...
const Receipt = () => {
    ...
    const openReport = () => {
        ...
        const pdfUrl = `https://5461283ce09c.ngrok-free.app/myapp/rptreceiptvoucher.php?com_code=${companyCode}&invcno=${invoiceNo}`;
    ...
    useEffect(() => {
        const fetchInvoices = async () => {
            try {
                const response = await fetch('https://5461283ce09c.ngrok-free.app/myapp/getreceipt.php');
    ...
    const renderInvoiceItem = ({ item }) => {
        if (!item || !item.rvno) return null; // Skip if data is incorrect

        return (
            <TouchableOpacity 
                onPress={() => { setInvoiceNo(item.rvno); setIsInvoiceModalVisible(false); }} 
                style={styles.invoiceItem}
            >
                <Text style={styles.invoiceItemText}>{item.rvno}</Text>
            </TouchableOpacity>
        );
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <LinearGradient colors={['#6200ee', '#3700b3']} style={styles.gradient}>
                <Text style={styles.heading}>Receipt Voucher</Text>
            </LinearGradient>
