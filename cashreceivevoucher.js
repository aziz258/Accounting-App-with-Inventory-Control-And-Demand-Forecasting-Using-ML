...
const [showModifyDeleteModal, setShowModifyDeleteModal] = useState(false); // Modal for modify/delete option
  ...
  useEffect(() => {
    axios
      .get("https://5461283ce09c.ngrok-free.app/myapp/fetchrvno.php")
      .then((response) => {
        if (response.data.success) {
          setOjvno(response.data.ojvno);
          setSelectedDate(new Date());
        } else {
          console.error("Failed to fetch RV No:", response.data.message);
        }
      })
      .catch((error) => {
        console.error("Error fetching RV No:", error);
      });
  ...
  const insertData = async () => {
    ...
    fetch("https://5461283ce09c.ngrok-free.app/myapp/insertrvno.php", {
      method: "POST",
      headers: {
...
>
  ...
  <Text style={styles.heading}>Cash Receipt Voucher</Text>
</View>
