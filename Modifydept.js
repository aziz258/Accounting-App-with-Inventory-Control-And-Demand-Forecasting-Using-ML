...
const ModifyDept = () => {
  ...
  const fetchGroupDetails = () => {
    ...
    if (!departmentNo) {
      Alert.alert("Error", "Please enter a Department No.");
      return;
    }

    // Send request to fetch group name and category based on group_no
    fetch("https://5461283ce09c.ngrok-free.app/myapp/fetchdeptdata.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ group_no: departmentNo }),  // Only send group_no
    })
      .then((response) => response.json())
      .then((data) => {
        if (data.success) {
          // If data is found, update the state
          setSelectedGroupNo(data.gcode);  // Set gcode
          setSelectedGroupName(data.gname);  // Set group name
          setCategory(data.category);  // Set category
          setDepartmentName(data.dname);
        } else {
          // If no data is found
          Alert.alert("Error", "No Department found with this Department no.");
        }
      })
      .catch((error) => {
  ...
  const handleSubmit = () => {
    ...
    fetch("https://5461283ce09c.ngrok-free.app/myapp/insertmodifydept.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.formContainer}>
        <View style={styles.headingContainer}>
          <Text style={styles.heading}>Modify Department</Text>
        </View>
        ...
        <View style={styles.inputRow}>
          <View style={styles.smallInputContainer}>
            ...
            <TextInput 
              ...
              value={departmentNo} 
              onChangeText={setDepartmentNo}  // Make it editable now
            />
          </View>
           <TouchableOpacity 
...
const styles = StyleSheet.create({
  ...
  smallInput: {
    ...
    borderRadius: 8,
    padding: 10,
    backgroundColor: "#fff",  // Made it editable
