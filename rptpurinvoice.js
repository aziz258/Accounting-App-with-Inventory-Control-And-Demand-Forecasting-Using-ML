...
const PurchaseInvoiceReport = () => {
    ...
    const openReport = () => {
        ...
        const pdfUrl = `https://5461283ce09c.ngrok-free.app/myapp/rptpurchaseinvoice.php?com_code=${companyCode}&invcno=${invoiceNo}`;
    ...
    useEffect(() => {
        const fetchInvoices = async () => {
            try {
                const response = await fetch('https://5461283ce09c.ngrok-free.app/myapp/frmpurinvoice.php');
    ...
    const renderInvoiceItem = ({ item }) => {
        if (!item || !item.grno) return null; // Skip if data is incorrect

        return (
            <TouchableOpacity 
                onPress={() => { setInvoiceNo(item.grno); setIsInvoiceModalVisible(false); }} 
                style={styles.invoiceItem}
            >
                <Text style={styles.invoiceItemText}>{item.grno}</Text>
            </TouchableOpacity>
        );
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <LinearGradient colors={['#6200ee', '#3700b3']} style={styles.gradient}>
                <Text style={styles.heading}>Purchase Invoice Report</Text>
            </LinearGradient>
